resource "azurerm_storage_account" "storage_account" {
    name                     = "storage${var.projectlower}${var.environment}2"
...
resource "azurerm_storage_container" "blob_dba_container" {
    name                  = "dbart"
...
resource "azurerm_private_endpoint" "blob_private_endpoint"{
    ...
    name = "blob-private-endpoint-${var.projectlower}-${var.environment}"
    resource_group_name = azurerm_resource_group.rg.name
    location = var.location
    subnet_id = azurerm_subnet.subnetapp.id

    private_service_connection {
        name = "storage-private-${var.projectlower}-${var.environment}"
...
resource "azurerm_private_endpoint" "queue_private_endpoint"{
    ...
    name = "queue-private-endpoint-${var.projectlower}-${var.environment}"
    resource_group_name = azurerm_resource_group.rg.name
    location = var.location
    subnet_id = azurerm_subnet.subnetapp.id
...
resource "azurerm_private_dns_a_record" "sa_private_dns_a_record"{
    ...
    name = "storage-record-${var.projectlower}-${var.environment}"
...
resource "azurerm_private_dns_zone_virtual_network_link" "sa_vnet_link"{
    name = "sa-vnetlink-${var.projectlower}-${var.environment}"
    resource_group_name = azurerm_resource_group.rg.name
